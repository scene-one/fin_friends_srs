...
- **판본** — v0.1 → v0.2(2026-08-21 가독성 개정: 모든 코드에 뜻을 병기하고 [§0 용어·코드 사전](#0-용어코드-사전--이-문서의-모든-약어) 신설): `PRD_핀프렌즈_v0_1_품질리뷰.md`의 **측정 가능성·검증 가능성 결함 11건(CRITICAL 6)** 을 적용(변경 요약 [부록 E](#부록-e-v01--v02-변경-내역)) → **v0.3**(2026-08-25 정합성 개정: 이미 확정된 결정에 남아있던 미결·모순 문구 제거 + 기능요구 스프린트 실현성·설계 결정 기록 보완): 변경 요약은 [부록 G](#부록-g-v02--v03-변경-내역)
...
### 🆕 4-1-1. Could 이상 기능의 스프린트 실현성

> ⚠️ **가정** — 팀 벨로시티가 아직 확정되지 않아 **1스프린트 = 2주**를 잠정 기준으로 판정했습니다. 실제 스프린트 계획 수립 시 재검토가 필요합니다. **Won't(F18)은 이번 릴리즈 대상이 아니므로 판정에서 제외**합니다.

| F | 난이도 | 스프린트 판정 | 사유 · 분해 필요사항 |
|---|:-:|---|---|
| F1 | 4 | ⚠️ **분해 필요(2스프린트)** | (1) 4칸 기본 렌더 + 실천근거 노출 → (2) 정체 원인 판정 로직. 한 스프린트에 묶으면 US-3 AC 검증이 늦어짐 |
| F2 | 3 | ✅ 1스프린트 | 조건 설정→승인→⭐ 지급, 단일 흐름 |
| F3 | 3 | ⚠️ **개발·콘텐츠 분리 트랙** | 퀴즈 엔진 자체는 1스프린트, 4주제 콘텐츠 제작(카피·검수)은 개발과 병행하는 별도 트랙으로 관리 필요 |
| F4 | 3 | ⚠️ **분해 필요(2스프린트)** | 트리거 8종 판정 로직은 1스프린트, **정합성 오류율 0%**(§5-2 불변 조건) 검증을 위한 이중기입·일일정산 배치는 별도 스프린트로 분리 권장 |
| F5 | 4 | ⚠️ **에셋 제작은 별도 트랙** | 3D 에셋(동물·옷) 제작은 디자인 리소스 트랙, 착탈·장착 로직 자체는 1스프린트 |
| F7 | 4 | ⚠️ **분해 필요(2스프린트)** | 5단계 온보딩 UI는 1스프린트, **법정대리인 동의 게이트 자동테스트 100%**(§5-1) 확보에 추가 스프린트 필요 — 규제 요건이라 축소 불가 |
| F8 | 5 | 🔴 **분해 필요(3스프린트)** | (1) 계획 카드 CRUD → (2) 결제 매칭 엔진(정확도 ≥90% 검증 포함, US-4 AC2) → (3) 두 갈래 회고 UX(US-5). 최고 난이도 기능을 1스프린트로 잡으면 매칭 정확도 미검증 상태로 출시될 위험 |
| F9 | 3 | ✅ 1스프린트 | 단, F1 나무 데이터에 의존하므로 **F1 완료 후 착수** |
| F6 | 2 | ✅ 1스프린트 | — |
| F10 | 2 | ✅ 1스프린트 | F4(별 지급 엔진) 완료 후 확장 형태로 착수 |
| F11 | 2 | ✅ 1스프린트 | 배치+알림 발송 표준 패턴 |
| F12 | 2 | ✅ 1스프린트 | — |
| F13 | 2 | ✅ 1스프린트 | F8의 결제 데이터에 의존하므로 **F8 이후 착수** |
| F15 | 3 | 🔴 **스프린트 편성 불가** | P-20(예적금 가입 중개 회피) 법률 검토 통과가 선행 조건(§7-3) — 난이도가 아니라 **외부 의존성**이 병목 |
| F16 | 3 | ✅ 1스프린트 | 온보딩 잠금 우회 로직 |
| F17 | 3 | 🔴 **스프린트 편성 불가** | P-21(별↔저금통 분리) 재검토가 선행 조건(§4-1 근거란) |

> **읽는 법** — Must 8개 중 3개(F1·F4·F7)와 최고난도 F8이 실제로는 **1스프린트가 아니라 분해된 다중 스프린트**로 진행돼야 검증 가능한 수준까지 완성됩니다. Could 3개 중 2개(F15·F17)는 스프린트 크기와 무관하게 **정책·법률 선결 조건**에 막혀 있어, "구현 가능성"보다 "착수 가능 시점"이 실제 병목입니다.
...
| **아동 계정** | id · 보호자_id · 생년(만 나이) · 기기 유형(전용폰/키즈워치/공유/없음) | 기기 유형은 **참고 정보로만 유지** — F14·F19 삭제 확정으로 대상 판정 용도는 소멸(§7-2 R2) |
...
> 🆕 이 절의 리스크·가정은 "무엇이 불확실한가"를 다룹니다. **"왜 이 구조로 설계했는가"**(F14·F19 삭제, 무료 정책, 폐쇄형 데이터 구조 등 이미 확정된 결정의 배경·트레이드오프)는 [부록 F. 주요 설계 결정 기록(ADR)](#부록-f-주요-설계-결정-기록-adr)에 모았습니다.
...
| **Out** *(이번 릴리즈)* | 📍 위치 알림(F14) · 예적금 실천(F15) · 카드 없는 체험(F16) · 별의 옷장 외 목적지(F17) · 기록 이전(F18) · **소비 사전 개입 자동 트리거(F19) — 삭제 확정, F8 계획 카드로 흡수**(§4-2) |
...
| 유형 | 내용 | 확정 방법 | 상태 |
|---|---|---|---|
| ~~의존성~~ | ~~아이 기기 알림 수신 가능 여부~~ | — | **해소**(2026-08-21) — F14·F19 삭제 확정으로 더 이상 의존성 아님(§7-2 R2) |
...
| ~~**E7**~~ | ~~아이 기기에 알림이 도달한다~~ | — | — | — | — | **해소** — F14·F19 삭제 확정으로 실기기 검토 불필요(§7-2 R2) |
...
| **② 개입 시점** | **사전 개입 수단 개수** | 0개 | 0개 | 0개 | **1개**(F8 계획 카드 — MVP Must 확정) | 기능 목록 대조 |
...
| 4 | **「소비 사전 개입」을 현재 제공 기능**으로 | *"사전 수단은 계획 카드(F8)뿐이며, 자동 개입 트리거는 도입하지 않기로 확정"* |
...
| **6** | **F14·F19 삭제, F8(계획 카드)로 흡수 확정**(2026-08-21 · §4-2) — 자동 사전 개입은 도입하지 않기로 결정. C5(사전 개입 사각지대)는 완전히 해소되진 않고 「계획을 적지 않은 소비」로 범위를 좁혀 §1-4에 남김 |
| **7** | **선결 검토 완료 → F14·F19 삭제 결정으로 귀결** — 기술 검토(기기 전제)와 체류시간 실측 선행 조건이 모두 미해결로 확인되자, 자동 트리거 대신 F8 계획 카드로 대체하기로 확정. 다음 단계는 계획 카드 작성률 실측(E3) → 프로토타입 → 인터뷰 |
...
**v0.2에서도 해결하지 못한 것 2건** *(② 개입 시점 축의 우위는 F14·F19 삭제 확정 + F8 계획 카드 채택으로 해소 — §4-2 · §8-3)*

| # | 내용 | 왜 |
|:-:|---|---|
| **1** | **확정 SLO** | 잠정 상한까지. 아키텍처·프로토타입 실측 없이 확정치를 쓰면 근거가 없습니다 |
| **2** | **대부분의 기준선** | `⬜ 미측정`은 인터뷰·프로토타입 전까지 해소되지 않습니다. v0.2는 **"어떻게 측정하는가"** 까지만 확정했습니다 |

---

## 부록 F. 주요 설계 결정 기록(ADR)

> 🆕 **v0.2에서 신설.** 본문 곳곳에 서술형으로 흩어져 있던 "왜 이렇게 설계했는가"를 한곳에 모았습니다. 각 항목은 배경(Context)·결정(Decision)·근거·결과(Consequences) 순으로 적었고, 본문 중복을 피하기 위해 세부 수치·표는 원문 절을 참조로 남깁니다. **여기 실린 결정은 전부 [설계 확정] 또는 ✅ 결정 완료 상태**이며, 목표 수치·기준선처럼 미실측 상태로 남은 항목은 포함하지 않습니다.

### ADR-001. 소비 사전 개입을 자동 트리거가 아닌 「계획 카드」로 구현

- **상태** — ✅ 결정 완료 (2026-08-21)
- **배경** — 위치 알림(F14)·소비 사전 개입 2차 트리거(F19)로 자동 개입을 구현하려 했으나, 선결 조건 4개(아이 기기 전제·체류시간 실측·P-19 온디바이스 판정 범위·온라인 결제 개입 수단) 중 3개가 미해결이었고, 선별 인물 H1·H2 둘 다 전용 스마트폰이 없어 알림 수신 전제 자체가 성립하지 않았음
...
- **결과(트레이드오프)** — 자동 발동이 사라져 사각지대가 「3분 미만 소비」에서 「계획을 적지 않은 소비」로 이동(§1-4). C10(FOMO 3초 판단)·C12(온라인 개입 부재)는 대응 기능 0개로 남음. 생존 조건은 계획 카드 작성률 ≥50%(O7·E3)
- **참조** — §4-2 · §4-4 · 부록 D

### ADR-002. 북극성 KPI로 WPA(주간 실천 인정 아동 비율)를 채택

- **상태** — [설계 확정]
- **배경** — 선언 ①(성장이 일어난다)이 선언 ②(그 성장이 보인다)의 전제이며, 아이가 실천하지 않으면 부모 화면은 "거짓말 없이 비어 있을 뿐"이라 어떤 전달 지표를 북극성으로 삼아도 ①이 죽으면 함께 죽음
- **결정** — 나무 5초 회상(O1)·다음달 충전율(O14)·7일 재접속률(O5) 대신 **실천 트리거 3종(미션 승인·소비 회고·위시리스트 달성) 기준 WPA**를 단일 북극성으로 채택
- **근거** — C2(실천 공백 → 나무 정체)가 AOS·DOS 두 지표에서 순서가 흔들리지 않는 유일한 항목이고, 나무 승급 조건에 실천 횟수가 포함돼 있어 WPA가 제품 전체가 파생되는 단일 원천 지표임
- **결과(트레이드오프)** — 학습 경로(온보딩·출석·퀴즈)는 분자에서 명시적으로 제외해 "접속만으로 오르는 활동량 지표"로의 퇴화를 차단. 예적금(F15)이 개통되면 WPA-v1(3종)→v2(4종)로 전환하며 4주간 병기 필요
- **참조** — §1-3
...
- **결정** — 제휴사(선불업 등록 보유) 카드 발행에 의존하는 폐쇄형 수집 구조를 채택 — 핀프렌즈는 브랜드·앱·학습·실천·별 원장을, 제휴사는 선불전자지급수단 발행·가맹점망·이용한도(P-17·P-18)를 각각 담당
- **근거** — 아동 신원 기반 표준 API 접근이 법적으로 막혀 있어 대안이 없음
- **결과(트레이드오프)** — 가맹점 분류 상세도가 제휴사 정책에 종속되고, 이 상세도가 F8 회고 품질을 좌우함(§6-3). 카드 한도·업종 제한도 우리가 정할 수 없는 제휴사 종속 변수
- **참조** — §6-2 · §6-3
...
- **결과(트레이드오프)** — 별 원장은 이중 기입 + 일일 정산 배치로 정합성 오류율 0%를 불변 조건으로 검증해야 함(§5-2) — 이 검증이 F4(별 지급 엔진)의 실제 개발 부담을 키움(§4-1-1)
- **참조** — §5-1 · §5-3 · 부록 B

### ADR-005. 승인 지연 시 완료 시점 기준 소급 지급 + 주기(cycle) 귀속 분리

- **상태** — ✅ 결정 완료 (US-6 AC-E2)
- **배경** — 미션 승인은 8종 트리거 중 유일한 수동 판정 경로라 병목이 되며, 부모 승인이 늦으면 아이의 실천이 "안 한 것"으로 잘못 기록될 위험이 있음. 특히 승인이 다음 정산 주기로 넘어가면 별 지급과 나무 승급 조건의 귀속 시점이 어긋남
- **결정** — ⭐는 **완료 시점** 기준으로 소급 지급하되, **나무 승급 조건은 완료 시점이 속한 주기(cycle_id)에 귀속**시키고, 그 주기가 이미 종료됐다면 나무가 아니라 월간 숲 스냅샷에만 반영
- **근거** — 별 지급(아이 보상)과 나무 조건 반영(부모용 성장 증거)의 시간 축이 다르므로 하나의 규칙으로 묶으면 둘 중 하나가 왜곡됨
- **결과(트레이드오프)** — 소급 지급 성공률 100%를 불변 조건으로 요구(§5-2). 일괄 승인 시에도 건별 개별 소급 처리가 필요해 구현 복잡도가 증가(US-6 AC-E3)
- **참조** — §3 US-6 · 부록 C

### ADR-006. 확정 SLO 대신 「AC 역산 잠정 상한」으로 성능·가용성 정의

- **상태** — [설계 확정](방법론) · 각 수치 자체는 프로토타입 실측 후 확정 SLO로 교체 예정
- **배경** — v0.1은 성능·가용성이 전부 `⬜ 미정`이라 테스트 대상 자체가 존재하지 않았음. 그렇다고 근거 없이 확정치를 지어내면 실측과 무관한 숫자가 됨
- **결정** — 확정치를 만드는 대신, **AC가 성립하기 위해 반드시 지켜야 하는 상한**(예: 나무 렌더 p95 ≤1,250ms — US-1 AC1의 "5초 노출"의 25%를 넘으면 회상 테스트 자체가 오염)을 역산해 "잠정 상한"으로 설정
- **근거** — AC가 성립하지 않는 값은 어떤 경우에도 허용될 수 없으므로, AC 역산치는 근거 없는 추정치보다 신뢰할 수 있는 하한선(=상한) 역할을 함
- **결과(트레이드오프)** — 이 값은 SLO가 아니라 "이 값을 넘으면 AC가 성립하지 않는다"는 상한일 뿐이므로, 프로토타입 실측 3일 연속 초과 시 아키텍처 재검토 후 확정 SLO로 교체하는 재설정 트리거를 함께 정의
- **참조** — §5-2

---

## 부록 G. v0.2 → v0.3 변경 내역

> 근거 — `/aztks` 최종검토(2026-08-25): 6항목 기준표(목표·지표 / 스토리·AC / 기능요구 / 비기능 / 리스크·가정 / 범위) 대비 완성도 점검에서 드러난 보완 사항 적용

| # | 변경 | 위치 |
|:-:|---|---|
| 1 | **F14·F19 삭제(§4-2)가 이미 확정됐는데도** 다른 9개 절이 "결정 대기·블로커·미확인"으로 옛 상태를 남겨 결정의 힘을 스스로 훼손하던 모순 제거. **미실측 KPI 수치·인터뷰 0건 기반 페르소나의 [가설]·⬜ 등급 표기는 그대로 유지**(§9-2 인용 금지 정책과 충돌하므로 손대지 않음) | §6-1 · §6-3 · §7-1 · §7-3 · §8-2 · §8-3 · §9-2 · 최종 요약 · 부록 E |
| 2 | **Could 이상 기능의 스프린트 실현성 신설** — F1·F4·F7·F8은 난이도상 1스프린트로는 검증 가능한 수준까지 완성되지 않아 분해 필요, F15·F17은 스프린트 크기와 무관하게 법률·정책 선결 조건(P-20·P-21)에 막혀 있음을 명시 | §4-1-1 |
| 3 | **설계 결정 기록(ADR) 6건 신설** — F14·F19 삭제, WPA 채택, 폐쇄형 카드 구조, 별↔저금통 분리, 소급 지급 규칙, 잠정 상한 방법론의 배경·근거·트레이드오프를 서술형 산재 상태에서 한곳으로 정리 | 부록 F |
