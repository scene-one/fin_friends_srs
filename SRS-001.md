...
| 끊김 방지 | 승인 지연 시 소급 지급(F10), 3일 미접속 알림(F11) |
...
| 아이 정지→부모 인지 일수(O13) | ≤3일 | REQ-NF-009 |
...
| R8 | 아이 Pain이 부모 화면에 신호로 나타나지 않음(12건 중 0건) | F11(REQ-FUNC-039~044)을 Should에서 내리지 않음 |
...
| inactivity_notified | 미접속 알림 발송 | parent_id, child_id, last_session_at, sent_at, opened_at | O13 |
...
추가적인 상세 시퀀스(계획 카드→매칭→회고, 미션 승인→소급 지급, 3일 미접속 알림)는 §6.3에서 정의한다.
...
| REQ-FUNC-039 | F11 | Should | The system shall send an inactivity notification to the parent with 100% delivery rate once 72 hours elapse without a child session, achieving ≤3-day detection-to-awareness. | Given 최종접속 후 72시간 / When 배치 실행 / Then 발송률 100%, 인지 ≤3일 | US-7 AC1 |
| REQ-FUNC-040 | F11 | Should | The system shall include the child's stalled slot/condition in the inactivity notification content. | Given 알림 수신 / When 부모가 알림 열람 / Then 멈춘 지점 표시 | US-7 AC2 |
| REQ-FUNC-041 | F11 | Should | The system shall allow the parent to configure notification send-time windows and achieve ≥50% notification open rate. | Given 야간 확인만 가능한 부모 / When 알림 발송 / Then 시간대 조정 가능, 열람률 ≥50% | US-7 AC3 |
| REQ-FUNC-042 | F11 | Should (Exception) | The system shall fall back to an in-app banner and (with consent) SMS when push notifications are blocked, and shall measure the blocked state separately. | Given 푸시 차단 / When 72h 판정 / Then 배너+문자 대체, 차단상태 계측 | US-7 AC-E1 |
| REQ-FUNC-043 | F11 | Should (Exception) | The system shall branch the notification copy to a "reinstall" message and log it under a distinct event code when the child has uninstalled the app. | Given 앱 삭제 / When 72h 시점 / Then "재설치 안내" 분기, 다른 이벤트코드 | US-7 AC-E2 |
| REQ-FUNC-044 | F11 | Should (Exception) | The system shall NOT send an inactivity notification if the child reconnects at the 71-hour mark, achieving a 0% false-positive rate. | Given 71시간 시점 재접속 / When 배치 실행 / Then 미발송, 오탐 0건 | US-7 AC-E3 |
...
| REQ-NF-009 | Performance | 3일 미접속 알림 발송 지연 | p95 ≤ 6 h/주 | PRD §5-2 |
...
| REQ-NF-026 | Monitoring | 미접속 알림 발송 실패 대응 | >0건 시 4시간 내 대응, 24시간 미해결 시 대체 채널 강제 | PRD §5-5 |
...
| US-7 AC1 | REQ-FUNC-039 | TC-039 |
| US-7 AC2 | REQ-FUNC-040 | TC-040 |
| US-7 AC3 | REQ-FUNC-041 | TC-041 |
| US-7 AC-E1 | REQ-FUNC-042 | TC-042 |
| US-7 AC-E2 | REQ-FUNC-043 | TC-043 |
| US-7 AC-E3 | REQ-FUNC-044 | TC-044 |
...
| 알림시간대설정 | string | 알림 발송 선호 시간대 | REQ-FUNC-041 |
    ...
    loop 주기 실행
        ...
        alt 72시간 경과 & 재접속 없음
            ...
            alt 푸시 알림 가능
                ...
                Notify->>P: 앱 내 배너 + 문자 대체(REQ-FUNC-042)
            end
        else 71시간대 재접속 확인
            Batch->>Batch: 알림 미발송(오탐 방지, REQ-FUNC-044)
        end
    end
...
| O13(정지→인지 일수) | `inactivity_notified` | REQ-FUNC-039, REQ-NF-009 |
