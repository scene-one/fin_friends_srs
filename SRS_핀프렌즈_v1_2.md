# 핀프렌즈(FinFriends) SRS v1.3 — Software Requirements Specification
...
- **최종 업데이트** — 2026-08-31
- **판본** — v1.0(2026-08-25 최초 변환) → v1.1(2026-08-26 §8 개정: 실험 설계·8슬롯 모집 계획을 KPI 카운팅 계획으로 교체) → v1.2(2026-08-26 기술 설계 다이어그램 12종 신설: Use Case·ERD·CLD·Component·Sequence·Flow Chart를 관련 챕터 본문에 배치해 배경지식 없는 독자를 위한 가독성 보강) → **v1.3**(2026-08-31 F11 · US-7 · 3일 미접속 알림 삭제 확정). 변환 이력은 [§13](#13-문서-변경-이력)
...
| | 내용 |
|---|---|
| **In** | 앱으로 충전된 용돈의 흐름 — 벌기 → 잘 쓰기 → 모으기 → 불리기 · 학습 4주제 + 퀴즈 · 실천 판정(미션·회고·위시리스트) · 별·옷장 · 성장 나무·월간 숲 · 소비 내역 · 부모 온보딩·동의 · 소급 지급 |
...
| ~~**F11**~~ | ~~🔔 아이 3일 미접속 알림~~ | — | — | — | — | ❌ **삭제** | §13 v1.3 변경 이력 참조 |
...
**집계** — Must 8 · Should 4 · Could 3 · Won't 1 = 16개
...
| 일반 공개 | ① 세그먼트 대상 | WPA≥55% 2주 연속 · 정합성 오류 0건 |
...
| R8 🔴 | 아이 Pain이 부모 화면에 신호로 안 나타남(12건 중 0건) | 아이가 멈춰도 최대 한 달 뒤 인지 — F11(3일 미접속 알림) 삭제로 완화 수단 없음(§13 v1.3) | 없음 — 대체 완화책 필요 시 별도 결정 |
...
| v1.3 | 2026-08-31 | **F11(🔔 아이 3일 미접속 알림) 삭제 확정.** US-7 전체(스토리·AC 6개·시퀀스 다이어그램)를 삭제하고, F11을 참조하던 모든 항목을 함께 정리 — §1.2(Scope In) · Component/Use Case Diagram의 Notify 노드·U11 · §1.3.2·§3.1.1 기능 목록(F14·F19와 동일하게 취소선 처리) · §3.1.2 스프린트 표 · §3.2 성능요구사항("미접속 알림 발송 지연") · §3.4.2 `inactivity_notified` 이벤트 · §4 모니터링("탐지" 계층) · §6.1 O13 · §6.2 보조 KPI("탐지" 축) · §8.1 KPI 카운팅 · §8.5 일반공개 게이트 · §9.1 근거표(F11 도입 전후 비교 행) · §10 R8(완화 수단이 사라졌음을 명시). Job J3(끊김 방지)의 "탐지" 하위 목표를 충족하는 기능이 현재 없음이 R8에 남음 |
