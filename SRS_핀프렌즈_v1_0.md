# 핀프렌즈(FinFriends) SRS v1.0 — Software Requirements Specification
...
- **최종 업데이트** — 2026-08-25
- **판본** — v1.0 — PRD v0.3을 SRS 형식으로 최초 변환. 새 요구사항을 만들지 않고 **PRD에 이미 작성된 내용만 재배치·재표기**했습니다. 변환 이력은 [§13](#13-문서-변경-이력)
...
- **생존 조건** — 계획 카드 작성률(O7·AC4·E3) ≥50% 미만이면 F8의 사전·사후 절반이 모두 무너짐.
...
| 대상 절 | 검증 방법 |
|---|---|
| 4.1 기능(§3.1) | US-1~US-8의 전체 AC·AC-E(43개) + 실험 E1·E2·E4·E5·E6·E8·E9(§8) |
...
| 슬롯 | 대상 | 확인할 것 |
|---|---|---|
| Core 1 | H1형 — 용돈앱 6개월 이상 | 금융 집중 요구·C1·C2·C3·기록이전 저항 |
| Core 2 | H2형 — 현금 용돈+고정 소비 동선 | 계획 카드 실제 작성 여부(C5) |
| Core 3 | H3형 — 조건부·협상 지급 | 미션 루프가 「벌기」 근거로 작동하는가 |
| Adjacent 1 | H5형 — 저축 목표 없는 가정 | 위시리스트 단계 보상·A1 |
| Adjacent 2 | H6형 — 통제가 목적인 가정 | 타깃 경계 재확인 |
| Extreme 1 | H7형 — 부모 응답 48시간+ | 소급 지급 신뢰 차이 |
| Extreme 2 | H4형 — 온라인·게임 결제 주 소비 | C10·C12 발생률·아이 이탈 축 |
| Non-user 1 | H10형 — 무료 대체재 사용 중 | 카드·자녀 연결 의향 |

### 8.2 실험별 가설 · 설계 · 판정

| # | 가설 | 측정 도구 | PASS | HOLD | FAIL → 조치 |
|:-:|---|---|:-:|:-:|---|
| E1 | 나무가 「변화」를 전달한다 | rubric·κ≥0.6 | ≥6/8 | 4~5/8 | ≤3/8 → 나무 UI 재설계 |
| E2 | 실천 조건이 전달된다 | 개입 횟수 | ≥6/8 | 4~5/8 | ≤3/8 → 실천 근거 노출 강화 |
| E3 🔴 | 계획 카드가 실제로 작성된다 | `plan_card_created`÷`payment_settled` | ≥50% | 30~49% | <30% → F8 사전 절반 붕괴, UX 재설계 |
| E4 | 첫 실천이 7일 내 일어난다 | `practice_credited` 코호트 | ≥60% | 40~59% | <40% → 난이도 재조정 |
| E5 | 정체 원인 표시가 오귀인·불신을 줄인다 | 발화 코딩 | 각 ≤2/8 | 각 3/8 | ≥4/8 → 문구 재설계 |
| E6 | 부모가 금융 집중을 원한다 | (a)/(b)/(c) 분기 | (a)≥5/8 | (a)3~4/8 | (c)≥3/8 → 포지션 변경 |
| ~~E7~~ | ~~아이 기기에 알림이 도달한다~~ | — | — | — | 해소 — F14·F19 삭제로 불필요(§11.1) |
| E8 | 회고가 형식적 클릭으로 퇴화하지 않는다 | dwell_ms p50·재노출률 | p50≥3초·재노출≤2/8 | p50 2~3초 | p50<2초 → 문장풀 확대 |
| E9 | 아이 정지를 부모가 3일 내 안다 | 발송률·인지일수 | ≤3일·열람≥50% | 4~7일 | >7일 → 알림 재설계 |
| E10 🔴 | 실천 증가가 실제 소비·저축 행동 변화로 이어진다 | 3개월 종단 3지표 | 2/3 이상 개선 | 1개만 개선 | 불변 → 선언① 「성장」 재정의 |
| E11 | 국내 3사의 「학습→성장」 공백이 유지된다 | 분기 1회 재점검 | 공백 유지 | 부분 변화 | 경쟁사가 메움 → 포지션 재검토 |
...
```mermaid
flowchart LR
    A["① 결제 매칭 검증<br/>매칭 정확도 ≥ 90% (US-4 AC2)"] --> B["② 계획 카드 작성률 실측<br/>인앱 · MVP 4주 (E3 · O7)"]
    B --> C["③ 업종 일치를 ⭐ 판정에<br/>넣을지 결정 (US-4 AC-E2)"]
    C --> D["④ 프로토타입 1~2주<br/>Core 3 · Adjacent 2 (E4 · E8)"]
    D --> E["⑤ 인터뷰 8슬롯<br/>E1 · E2 · E5 · E6 · E9"]
    E --> F["⑥ 기준선 실측치 반영<br/>목표값 확정"]
...
| β 클로즈드 | 모집 8슬롯 가정 | E4 PASS(≥60%) · E1 PASS(≥6/8) · WPA≥5/8 |
...
| 주장 | 등급 | 검증 설계 | 통과 기준 |
|---|:-:|---|---|
| 「가르치는 것」과 「행동을 바꾸는 것」은 다른 문제다 | [A] | E10(3개월 종단) | 월간 숲 3지표 중 2/3 이상 개선 |
| 국내 3사 전부 「학습→성장」 공백이다 | [관측] | E11(분기 재점검) | 화면 캡처+약관 대조 |
| 퍼핀 부모 화면은 숫자 3개뿐이다 | [관측] | E11 포함 | 스크린샷 대조 |
| 실천 없이는 나무가 자라지 않는다 | [설계 확정] | US-2 AC2·AC-E3 단위테스트 | 실천 0건 승급 0건 |
| 퍼핀은 7과목 중 4개가 비금융이라 4칸 매핑 불가 | [관측] | E11 포함 | 비금융 비율 재산정 |
| 인센티브는 양, 내재동기는 질을 예측한다 | [A] | E8+WPA 제외규칙 검증 | 출석률과 WPA 상관 r≤0.3 |
| ① 성장증명형 29.3만명(35.8%)이 최대 세그먼트 | [A]+모델값 | 연1회 재산출 | 독립성 가정 유지 여부 명시 |
| C1·C2가 DOS 양쪽 1위, C5가 AOS 1위 | [추론] | 인터뷰 후 재채점 | 순위 변동 여부만 판정 |
| 부모 1순위 교육↔아이 실제 행동 격차 53.0%p | [A] | E10에 저축률 지표 편입 | 3개월 후 개선 여부 |
| 83.9%가 이미 계좌·카드형 대체재를 쓴다 | [관측] | 인터뷰 스크리너 교차확인 | 표본 편중 여부 판정 |
| 결제 인프라는 이미 깔려 있다(충전식 카드 64.2%) | [A] | 카드 연결률 대조 | ≥50% 시 전제 지지 |
| 아이 Pain 12건 중 부모 화면 신호 0건 | [추론] | E9(F11 도입 전후 탐지지연 비교) | 인지일수 ≤3일 |
| H1·H2의 Job과 Pain | ✍️[합성 가설]+🧪[모의] | 인터뷰 8슬롯 | κ≥0.6 |
...
| # | 리스크 | 영향 | 완화 |
|:-:|---|---|---|
| R1 🟠 | C5에 F8 계획 카드가 부분 대응 — 남은 것은 자동 발동 부재 | 작성률이 낮으면 사전·사후가 함께 무너짐 | 작성률을 MVP 4주 실측(O7·E3) |
...
- **결과(트레이드오프)** — 자동 발동이 사라져 사각지대가 「3분 미만 소비」에서 「계획을 적지 않은 소비」로 이동(§7.5). C10·C12는 대응 기능 0개로 남음. 생존 조건은 계획 카드 작성률 ≥50%(O7·E3)
...
이 SRS는 `PRD_핀프렌즈_v0_3.md`를 **1회 변환**한 v1.0이다. PRD 자체의 v0.1→v0.2→v0.3 변경 이력(측정 가능성 결함 11건 수정, F14·F19 삭제 후 모순 정리, 스프린트 실현성·ADR 추가 등)은 **원본을 재작성하지 않고 참조**한다 — `PRD_핀프렌즈_v0_3.md` 부록 E·부록 G 참조.
